...
class TranslationSettings(BaseModel):
    enabled: bool = Field(
        False,
        description="Use translate or not"
    )
    endpoint: str = Field(None, description="endpoint to connect with translation service")

class LLMSettings(BaseModel):
    mode: Literal[  
        "llamacpp", "openai", "openailike", "azopenai", "sagemaker", "mock", "ollama", "vllm", "nvidia_nim"
    ]
...
class EmbeddingSettings(BaseModel):
    mode: Literal["huggingface", "openai", "azopenai", "sagemaker", "ollama", "mock", "text_embeddings_inference"]
...
class RerankSettings(BaseModel):
    ...
    mode: Literal["text_embeddings_inference", "model_api", "local"] = Field(
        "model_api",
        description="Mode to serve rerank model"
    )
    top_n: int = Field(
        3,
...
class RagSettings(BaseModel):
    ...
    retrieval_top_k: int = Field(
        5,
        description="This value controls the number of documents after retrieval"
    )
...
class WeaviateSettings(BaseModel):
    weaviate_endpoint: str = Field(
        "The server hosting the Weaviate vector database"
    )
    index_name: str
