...
test-coverage:
	PYTHONPATH=. poetry run pytest tests --cov src --cov-report term --cov-report=html --cov-report xml --junit-xml=tests-results.xml
...
dev-windows:
	(set PGPT_PROFILES=local & poetry run python -m uvicorn src.main:app --reload --port 8001)
...
api-docs:
	PGPT_PROFILES=mock poetry run python scripts/extract_openapi.py src.main:app --out fern/openapi/openapi.json
